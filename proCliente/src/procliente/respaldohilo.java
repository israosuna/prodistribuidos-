 ...
 * @author daniel
 */
public class respaldohilo extends Thread {
    ...
    public respaldohilo(String ip_nodo_siguiente, int puerto_entrada, int puerto_salida, File archivo,String user) {
    ...
    public void run() {
      ...
      this.replicarArchivoXML();
    }

    public void replicarArchivo() {
        try {
            
            Socket socket = new Socket(ip_nodo_siguiente, puerto_salida);
    ...
    public void replicarArchivoXML() {
        try {
            ...
            //write file name
             servicioSocket.writeUTF("xml");
             
             
            servicioSocket.writeUTF("ListaArchivos.xml");
             
           //usuario o carpeta que debe poner el archivo
          
            servicioSocket.flush();
             FileInputStream archivo2 = new FileInputStream("../proCliente/ListaArchivos.xml");
            //buffer for file writing, to declare inside or outside loop?
             byte[] buffer = new byte[10000];
            //outer loop, executes one for each file
             System.out.println("ListaArchivos.xml");
              int len;
              //int total = 0;
            //write file to servicioSocket
           while ((len = archivo2.read(buffer)) != -1) {
